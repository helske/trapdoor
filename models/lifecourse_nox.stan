// Model for FSD data, model Z with G, S
...
model {
  ...
  for(i in 1:n) {
    ...
    real mu_grade = inv_logit(a_z + b_zg * gender[i] + b_zs * itpa[i]);
    
    real phi_grade = exp(a_z_phi + b_zg_phi * gender[i]);
...
generated quantities {
  ...
  for(k in 1:3) {  
    ...
    for(i in 1:N) {
      ...
      real phi_grade = exp(a_z_phi + b_zg_phi * gender_i);
