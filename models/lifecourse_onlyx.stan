// Model for FSD data, model Z with X, G, S
...
model {
  ...
  for(i in 1:n) {
    ...
    real mu_grade = inv_logit(a_z + b_zx * mo(simplex_zx, education[i]));
    
    real phi_grade = exp(b_zx_phi[education[i]+1]);
...
generated quantities {
  ...
  for(k in 1:3) {  
    ...
    vector[M * N] sample;
    vector[M * N] weight;
      real mu_grade = inv_logit(a_z + b_zx * mo(simplex_zx, k - 1));
      real phi_grade = exp(b_zx_phi[k]);
    for(i in 1:N) {
